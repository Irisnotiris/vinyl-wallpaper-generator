...
Lower-center of the image (positioned in the lower 45% of the canvas, horizontally centered, sitting just above the player bar): The album cover art for '{album_title}' by {artist_name}. {cover_visual_description — primary anchor, visual treatments, symbolic anchor, composition}. At the top of the cover, '{artist_name}' and below the main visual, '{album_title}' — both in a typography style chosen to match the source photo's mood and atmosphere (e.g., Nunito/Quicksand for warm/healing, Playfair Display/Bodoni for retro/nostalgic, Montserrat/Orbitron for cool/futuristic, Inter/Helvetica for calm/minimal, Didot/Cormorant for artistic/quiet, Cinzel/Trajan Pro for classical/poetic, Caveat/Dancing Script for youthful/romantic, Oswald/Bebas Neue for dark/industrial). The cover is a clean square with subtle paper edge thickness and soft natural contact shadow. {print_texture_description}.
